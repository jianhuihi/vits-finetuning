...
from pypinyin import lazy_pinyin, BOPOMOFO, Style
import jieba, cn2an
...
def chinese_to_pinyin(text):
  ...
  text=''
  for word in words:
    pingyin=lazy_pinyin(word, style=Style.TONE3, neutral_tone_with_five=True)
    if not re.search('[\u4e00-\u9fff]',word):
      text+=word
      continue
    for i in range(len(pingyin)):
      if re.match('[\u3105-\u3129]',pingyin[i][-1]):
        pingyin[i]+='ˉ'
    if text!='':
      text+=' '
    text+=''.join(pingyin)
...
def chinese_to_bopomofo(text):
  ...
  text=''
  for word in words:
    bopomofos=lazy_pinyin(word, style=Style.TONE3, neutral_tone_with_five=True)
    if not re.search('[\u4e00-\u9fff]',word):
      text+=word
      continue
